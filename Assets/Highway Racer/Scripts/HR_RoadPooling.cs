...
public class HR_RoadPooling : MonoBehaviour {
    ...
    private float GetRoadLength(GameObject road) {
        ...
        foreach (Renderer render in renderers) {
            ...
            if (render != roadReference.GetComponent<Renderer>() && 
                1 << render.gameObject.layer == asphaltLayer)
    ...
    private void CreateRoads() {
        ...
        for (int i = 0; i < roads.Count; i++)
        {
            ...
            if (i != 0)
            {
                ...
                //     roads[i - 1].transform.position.z +
                //     roadLength[
                //         index <= 0
                //         ? roadObjects.Length - 1
                //         : index - 1
                //     ]);
                ...
                if (index <= 0)
                {
                    zPos = (roadLength[i] / 2) + (roadLength[roadObjects.Length - 1] / 2);
                }
                else
                {
                    zPos = (roadLength[i] / 2) + (roadLength[index - 1] / 2);
                }
                
                roads[i].transform.position = new Vector3(0f, roads[i].transform.position.y,
                    roads[i - 1].transform.position.z + zPos);
            }
